...
module.exports = function (message, response) {
	...
	*/
	function callIrail (message, response) {
		...
		sendiRailrequest (fromStation, destStation).then((output) => {
			botResponse =  buildResponse (output, nextTrain);
			nextTrain = String (parseInt (nextTrain) + 1 );
			sendResponse(botResponse, nextTrain); // Send simple response to user
		});
		
		// Function to send correctly formatted responses to Dialogflow which are then sent to the user
		function sendResponse (responseToUser, nextTrain) {
			let responseJson = {};
